...
@media (max-width: 640px) {
    ...
    .pill-value { font-size: 13px; } .pill-label { font-size: 9px; letter-spacing: 0.2px; }
    ...
    if df.empty:
        ...
        if not exp_df_raw.empty:
            ...
            toggle_html = (
                ...
                f'<a href="{groups_url}" target="_self" class="{"active" if view_mode == "groups" else ""}">Группы</a>'
                f'<a href="{subcats_url}" target="_self" class="{"active" if view_mode == "subcats" else ""}">Детально</a>'
                '</div>'
            )

            st.markdown(
                '<div class="section">'
                '<div><div class="section-title">🎯 Структура расходов</div>'
                '<div class="section-sub">Нажми на категорию для деталей</div></div>'
                f'{toggle_html}'
                '</div>', unsafe_allow_html=True
            )
            ...
            with col_l:
                ...
                for i, (disp_cat, val) in enumerate(cats.items()):
                    ...
                    lparts.append(f'<a href="{url}" target="_self" class="cat-link{active}"><div class="cat-row">'
                        f'<div class="cat-icon" style="{icon_3d_style(color)}">{cat_icon(disp_cat)}</div>'
                        f'<div class="cat-info"><div class="cat-name">{cat_safe}</div>'
                        f'<div class="cat-bar-wrap"><div class="cat-bar" style="width:{pct:.1f}%; {bar_gradient(color)}"></div></div></div>'
                        f'<div class="cat-right"><div class="cat-amount">{fmt(val)} ₽</div><div class="cat-pct">{pct:.1f}%</div></div>'
                        '<div class="cat-chev">›</div></div></a>')
        ...
        if selected_cat:
            ...
            if tx_df is not None and not tx_df.empty:
                ...
                dparts = ['<div class="tx-card">', '<div class="tx-header">',
                    ...
                    '</div>',
                    f'<a href="{back_url}" target="_self" class="tx-back" title="Назад">←</a>',
                    '</div>']
